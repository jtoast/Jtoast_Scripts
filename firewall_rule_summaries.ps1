...
# Your previous script code goes here...

foreach ($Policy in $PolicyList) {
    # Get policy name using policy ID
    $PolicyName = ($PolicyList | Where-Object { $_.id -eq $Policy.id }).name
    $Param = @{
        Path = Join-Path $ExportPath "rule_summary_$($PolicyName).csv"  # Use policy name instead of ID
        Append = $true
    ...
    foreach ($GroupId in $Policy.rule_group_ids) {
        ...
        foreach ($RuleId in $Group.rule_ids) {
            ...
            $RuleName = $Rule.name
            [PSCustomObject]@{
                policy_name = $PolicyName  # Use policy name instead of ID
                rule_group_name = $GroupName  # Use group name instead of ID
                rule_id = $Rule.family
                policy_enabled = $Policy.enabled
                group_enabled = $Group.enabled
                rule_enabled = $Rule.enabled
                platform = $Group.platform
                rule_name = $RuleName  # Use rule name instead of ID
            ...
            } | Select-Object -Property * -ExcludeProperty rule_version | Export-Csv @Param -Force
        }
    }
...
# Your subsequent script code goes here...
